...
public interface UserRepository extends JpaRepository<Userco2,Long> {
    ...
    @Query("SELECT u FROM Userco2 u INNER JOIN u.roles r LEFT JOIN u.bilan b WHERE r.name = :roleName ORDER BY b.sum_total ASC")
    List<Userco2> findAllUserByRoleOrderSum_total(@Param("roleName") RoleName roleName);
